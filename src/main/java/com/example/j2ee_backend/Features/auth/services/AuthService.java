package com.example.j2ee_backend.Features.auth.services;

import com.example.j2ee_backend.Domain.entities.Account;
import com.example.j2ee_backend.Features.auth.dtos.*;
import com.example.j2ee_backend.Features.auth.repositories.AccountRepository;
import com.example.j2ee_backend.Features.auth.repositories.RefreshTokenRepository;
import com.example.j2ee_backend.Features.auth.utils.OtpProvider;
import com.example.j2ee_backend.Features.auth.utils.PasswordHasherBcrypt;
import com.example.j2ee_backend.Features.cache.CacheService;
...
import com.example.j2ee_backend.SharedKernel.security.JwtTokenProvider;
...
public class AuthService 
{
    ...
    {
        var accountExisted = _accountRepo.findByUsername(request.username);
        if(accountExisted.isPresent()) throw new APIException(HttpStatus.BAD_REQUEST.value(), "Account is existed");
    ...
    public APIResponse<Map<String, Object>> authenticate(LoginRequest request) {
        ...
        var account = _accountRepo.findAccountRole(request.username);
